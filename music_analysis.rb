...
#
# Given an array where each element in the array is an ordered voice of pitches
# (another array), permutate each row using rotation and search each column for
# a pattern of sonorities from a dictionary. If a sonority is found, then
# tabulate a score for reporting.
#
# Useful for counterpoint such as creating a canon, composing 12t, or set
# related composition designs
#
# NOTE: Performance is an issue with larger number of voices and pitches:
#      x^(n-1) where x = number of pitches in a row/voice and n is the number
#      of rows/voices.
#
#      For example:  50 pitches in 8 voices would generate 50^7 permutations
...
#    analysis_engine.run()
#
#    Output sample - one detail analysis of a possible rotation and
#    the run summary:
#
...
#                       ex: the detail above
#
#    ex:  there are 42 permutation solutions that have 5 columns holding
...
class MatrixAnalyzer
  attr_accessor :horizontal_sets # A 2d array for holding horizontal ordered sets
                                 # (rows) - each set:
                                 # 1) representing an ordered voice of pitches
                                 #    in the matrix
                                 # 2) should be rotatable like a cannon
                                 # 3) encoded in mod12 pitch class notation
                                 #    (0 = c up to 11 = b)

  attr_accessor :vertical_sets   # A 2d array for holding set of pitches to
                                 # analyze when looking for vertical
                                 # sonorities.  These should be transformed
                                 # using Tn or TnI

  attr_accessor :minimum_score   # Minimum score that must be met in the
                                 # results for a solution to be displayed.
                                 # default = 0

  attr_accessor :maximum_score   # Maximum score that the result must be under
                                 # for a solution to be displayed.
                                 # default = 99999999

  attr_accessor :report_details  # If true, show details of the analysis,
                                 # else summary only. default = false

  attr_accessor :summary_totals  # Used to collect a final tally of subsets
                                 # found. For example:
                                 # summary_totals[9] = 21 means 21 permutations
                                 # had 9 subsets found in each
  public
  ...
  #
  # * *Parameters*    :
  #   - +horizontal_sets+ [Array] -> An array of voices
  #   - +vertical_sets+ [Array] -> An array of search sets
  ...
  def initialize(horizontal_sets = Array.new(),
      vertical_sets = Array.new(),
      min_score = 0,
      max_score = 99999999,
      report_details = false)
    self.horizontal_sets=(horizontal_sets)
    self.vertical_sets=(vertical_sets)
    self.minimum_score=(min_score)
    self.maximum_score=(max_score)
    self.report_details=(report_details)
    self.summary_totals=(Array.new())
  end
  ...
  def add_horizontal_set(horizontal_set)
    ...
    self.horizontal_sets() << horizontal_set
  end
  ...
  def add_vertical_set(vertical_set, transpose = 0)
    ...
    self.vertical_sets() << Set.new(vertical_set)
  end

  # Run the analysis and print out the results
  #
  # Non recursive option is only there to compare performance of two coding
  # techniques, and they are not all that different in performance,
  # surprisingly.
  #
  # * *Parameters* :
  ...
  def run(run_recursively = true)
    self.summary_totals().clear()
  ...
  # A recursive routine that rotates a voice in the horizontal matrix of
  # voices.  If it is at depth, performs the final vertical analysis for the
  # current rotations; otherwise, just calls itself to process the next row
  # in the hierarchy of voices.
  #
  # * *Parameters* :
  ...
  def rotate_via_recursion(level = -1)
    ...
    unless(level > 0)             # First voice remains static
      ...
      horizontal_sets   = self.horizontal_sets()
      max_row_index     = horizontal_sets.length() - 1
      row               = horizontal_sets[level] # Get row for this depth

      # Rotate each pitch to the right for this row.  If its the last row then
      # analyze the verticals in each column across all rows, otherwise
      # recursively call to process next row.
      row.each_index() do
        row.rotate!(-1)
  ...
  # Step through each column in the matrix of rotated rows (voices) and look
  # for patterns of sets from a dictionary to create a score for each column
  # (found) and for the entire matrix of rows (total found).
  #
  # * *Parameters* :
  #   - none
  # * *Returns* :
  ...
  def analyze_sonorities()
    horizontal_sets  = self.horizontal_sets()
    vertical_sets    = self.vertical_sets()
    sonority_to_test = Set.new()             # A work space for slicing
    result_counts    = Array.new()           # Success counters of columns

    # Loop on columns in the matrix and extract out the sonority. Compare that
    # to the dictionary of vertical sets to calculate a score for each column.
    horizontal_sets[0].each_index() do |i|
      ...
      #sonority_to_test.sort!()               # Put in low-high order to compare

      # Search the dictionary of vertical sets we are looking for and increment
      # column counter if found.
      vertical_sets.each{ |set_to_search| result_counts[i] += 1 if(sonority_to_test == set_to_search) }
    end

    # Accumulate a cross total of column result to create a final score for the
    # current rotation snapshot.
    total_common_sonorities = result_counts.inject(0){ |sum, col_result| sum += col_result }

    # If we meet the search criteria then add results to report totals and
  ...
  # Accumulate current snapshot search result counts into the reports summary
  # totals.  For example: if the total column score was 9 for the current
  # rotation of the rows, summary_totals[9] would be incremented to reflect
  # that a matrix solution was found with score = 9
  #
  # * *Parameters* :
  ...
  def accumulate_summary_totals(score)
    ...
    summary_totals = self.summary_totals()
    counter = summary_totals[score]
    counter = 0 if(counter.nil?)  # Initialize first use
    counter += 1
    summary_totals[score] = counter
  end
  ...
  def print_details(result_counts, score)
    ...
    raise ArgumentError, "score is mandatory" if(score.nil?)
    puts ('=' * 10) << "\n"
    self.horizontal_sets().each(){ |line| p line }
    puts  '-' *  (self.horizontal_sets()[0].length() * 3)
    p result_counts
    puts "\nTotal Score: " << score.to_s << "\n"
  end

  # A NON-recursive SINGLE routine that rotates a voice in the horizontal
  # matrix of voices. Exactly the same as rotate_via_recursion(),
  # analyze_sonorities(), accumulate_summary_totals() and print_details() all
  # in one package.  This is only for experimenting with maximum optimization
  # by eliminating invocations()
  #
  # * *Parameters* :
  #   - none
  # * *Returns* :
  ...
  def rotate_inline_experiment()
    ...
    while current_level > 0 do
      ...
      # BEGIN merge of analyze_sonorities() code run

      # Loop on columns in the matrix and extract out the sonority
      # Compare that to the dictionary of vertical sets to calculate a score
      # for each column...
      horizontal_sets[0].each_index() do |i|
        result_counts[i] = 0            # Initialize counter for column
        sonority_to_test.clear()
        ...
        # Search the dictionary of vertical sets we are looking for and
        # increment column counter if found.
        vertical_sets.each{ |set_to_search| result_counts[i] += 1 if(sonority_to_test == set_to_search) }
      end

      # Accumulate a cross total of column result to create a final score for
      # the current rotation snapshot.
      total_common_sonorities = result_counts.inject(0){ |sum, col_result| sum += col_result }
      ...
      if(total_common_sonorities >= self.minimum_score() && total_common_sonorities <= self.maximum_score())
        ...
        summary_totals = self.summary_totals()
        counter = summary_totals[total_common_sonorities]
        counter = 0 if counter.nil?               # Initialize first use
        counter += 1
        summary_totals[total_common_sonorities] = counter
  ...
  def print_summary()
    puts "\nScore : # Instances\n" << ("=" * 19)
    self.summary_totals().each_with_index { |value, index| puts " %5d:%8d\n" % [index, value] unless(value.nil?) }
    puts "\n** End of Report"
  end

  # Given a musical pitch, and how many 1/2 steps you want to transpose it,
  # returns a new pitch at the new transposition
  #
  # * *Parameters* :
