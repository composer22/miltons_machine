...
module MiltonsMachine
  ...
  class MatrixAnalyzer
    ...
    def add_row( group_id, row, number_to_transpose = 0 )
      ...
      @groups[group_id - 1] << MiltonsMachine::ForteSet.new(row).transpose_mod12(number_to_transpose)
    ...
    def add_search_set( search_set, number_to_transpose = 0 )
      @search_sets << Set.new( MiltonsMachine::ForteSet.new(search_set).transpose_mod12(number_to_transpose) )
    end
    ...
    def add_forte_set( forte_set_name, number_to_transpose = 0 )
      search_set = MiltonsMachine::ForteDictionary.instance.get_set(forte_set_name)
